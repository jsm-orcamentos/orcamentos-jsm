...
async function ensureSignedIn() {
  ...
  // Verifica se já estava logado antes (evita pedir consentimento desnecessário)
  const wasLogged = localStorage.getItem("jsm_google_logged") === "1";
  const promptType = wasLogged ? "" : "consent"; // silencioso se já logado, consentimento se primeira vez

  // 1) tenta silencioso (não abre abinha)
  const silentOk = await new Promise((resolve) => {
    tokenClient.callback = (resp) => {
      if (resp && resp.access_token) {
        accessToken = resp.access_token;
        window.gapi.client.setToken({ access_token: accessToken });
        localStorage.setItem("jsm_google_logged", "1"); // marca como logado
        resolve(true);
      } else {
        resolve(false);
      }
  ...
  // 2) fallback: pede consentimento (abre abinha só quando precisar)
